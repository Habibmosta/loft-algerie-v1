...
export async function getExecutiveMetrics(userId: string): Promise<ExecutiveMetrics> {
  ...
  const occupiedLofts = lofts?.filter((l: any) => l.status === 'occupied').length || 0
  const occupancyRate = totalLofts > 0 ? (occupiedLofts / totalLofts) * 100 : 0

  // Calculer les revenus par type de propriétaire
  const companyRevenue = lofts?.filter((l: any) => l.loft_owners?.ownership_type === 'company')
    .reduce((sum, l) => sum + (Number(l.price_per_month) * (l.company_percentage / 100)), 0) || 0
    
  const thirdPartyRevenue = lofts?.filter((l: any) => l.loft_owners?.ownership_type === 'third_party')
    .reduce((sum, l) => sum + (Number(l.price_per_month) * (l.company_percentage / 100)), 0) || 0
  ...
  return {
    ...
    totalLofts,
    occupancyRate,
    averageRentPrice: totalLofts > 0 ? lofts!.reduce((sum, l: any) => sum + Number(l.price_per_month), 0) / totalLofts : 0,
